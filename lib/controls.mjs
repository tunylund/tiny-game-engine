import { vector, xyz, zero, equal } from './xyz.mjs';
...
export function buildControls(window, onChange = () => { }) {
    let resetTimeout;
    const changeDetection = (fn) => (e) => {
        const beforeDir = xyz(controls.dir);
        const beforeCor = xyz(controls.cor);
        const beforeKeys = JSON.stringify(controls.keys);
        fn(e);
        if (!equal(beforeDir, controls.dir) || !equal(beforeCor, controls.cor))
            onChange(controls);
        else if (beforeKeys !== JSON.stringify(controls.keys))
            onChange(controls);
    };
    const center = () => xyz(window.innerWidth / 2, window.innerHeight / 2);
    const keyDown = changeDetection((e) => handleKey(e.code, true, controls));
    const keyUp = changeDetection((e) => handleKey(e.code, false, controls));
    const touchStart = changeDetection((ev) => resetTimeout = handleTouch(ev.changedTouches[0], center(), controls, resetTimeout));
