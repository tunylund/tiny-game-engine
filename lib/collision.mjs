...
import { xyz, sub, cross, equal, mul, negone, one, add } from './xyz.mjs';
// glory to http://www.jeffreythompson.org/collision-detection
export function entityCollidesWithPolygon2d(entity, polygon) {
    const entityPoly = [
        xyz(entity.pos.cor.x - entity.dim.x2, entity.pos.cor.y - entity.dim.y2),
        xyz(entity.pos.cor.x + entity.dim.x2, entity.pos.cor.y - entity.dim.y2),
        xyz(entity.pos.cor.x + entity.dim.x2, entity.pos.cor.y + entity.dim.y2),
        xyz(entity.pos.cor.x - entity.dim.x2, entity.pos.cor.y + entity.dim.y2)
    ].map(({ x, y }) => xyz(x * Math.cos(entity.dir.radian) - y * Math.sin(entity.dir.radian), x * Math.sin(entity.dir.radian) + y * Math.cos(entity.dir.radian)));
...
export function bump(otherEl, collidables = [], fixSelector = one) {
    ...
    if (correction.sum === 0) {
        ...
        return position(sub(otherEl.pos.cor, correction), mul(otherEl.pos.vel, add(mul(fixSelector, negone), one)), otherEl.pos.acc);
    }
}
//# sourceMappingURL=collision.mjs.map
