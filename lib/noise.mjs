...
function noise(cor, grid) {
    ...
    if (!grid[y0])
        grid[y0] = [];
    if (!grid[y0][x0])
        grid[y0][x0] = vector(Math.PI * 2 * Math.random(), 1);
    if (!grid[y0][x1])
        grid[y0][x1] = vector(Math.PI * 2 * Math.random(), 1);
    if (!grid[y1])
        grid[y1] = [];
    if (!grid[y1][x0])
        grid[y1][x0] = vector(Math.PI * 2 * Math.random(), 1);
    if (!grid[y1][x1])
        grid[y1][x1] = vector(Math.PI * 2 * Math.random(), 1);
