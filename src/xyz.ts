class XYZ {
  ...
  constructor(x: number|XYZ = 0, y = 0, z = 0) {
    if (x instanceof XYZ) {
      this.x = x.x
      this.y = x.y
      this.z = x.z
  ...
  get signature() { return xyz(
    this.x === 0 ? 1 : Math.abs(this.x) / this.x,
    this.y === 0 ? 1 : Math.abs(this.y) / this.y,
    this.z === 0 ? 1 : Math.abs(this.z) / this.z,
  )}
 ...
 * @param {number} [z]
 */
function xyz(x: number|XYZ = 0, y = 0, z = 0) { return new XYZ(x, y, z) }
function vector2(radian: number, size = 1) { return xyz(
  Math.round(size * Math.cos(radian) * 10000) / 10000,
  Math.round(size * Math.sin(radian) * 10000) / 10000,
)}
function unit(v: number) { return new XYZ(v, v, v) }
